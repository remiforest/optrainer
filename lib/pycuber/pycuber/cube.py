...
class Cube(object):
    ...
    def __init__(self, cubies=None):
        ...
        if not cubies:
            cubies = set()
            colours = {"F": "red", "U": "yellow", "R": "green", "D": "white", "B": "orange", "L": "blue"}
            for loc in [
                "LDB", "LFD", "LBU", "LUF", "RBD", "RDF", "RUB", "RFU",
    ...
    def __iter__(self):
        ...
        result = []
        for loc in [
            "BLD", "FDL", "UFL", "ULB", "RFU", "RUB", "RBD", "RDF",
    ...
    def colors(self, position):
        """
        Returns the colors of the Cubie at the given position as a string.
        The colors are returned in the order of the faces as they appear in the position string.
        """
        try:
            cubie = self[position]  # Utilise __getitem__ pour récupérer le Cubie
            # Extraire les couleurs dans l'ordre des faces spécifiées dans 'position'
            # print(cubie)
            # for face in position:
                # print(face)
            colors = ''.join(cubie.facings[face].colour[0] for face in position)
            return colors
        except KeyError:
            return "Position not found in the cube"
        except IndexError:
            return "Invalid face in position"
